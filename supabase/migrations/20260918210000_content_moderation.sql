...
-- a human look.
--
-- Port of recovery 20260633120000_content_moderation.sql with a later
-- timestamp so it runs after 20260918120000 identity/city hardening.
-- Difference from that original: screen_profile_text flags contact_details
-- (queue + allow write) instead of blocking, matching chat.
...
-- Screen public profile text. Hard-block categories still reject the write.
-- contact_details (action = flag) is queued and allowed, matching chat.
...
begin
  ...
  loop
    if normalised ~ rule.pattern or combined ~* rule.pattern then
      ...
      insert into public.moderation_queue (subject_type, subject_id, author_id, category, excerpt)
      values ('profile', new.id, new.id, rule.category, left(combined, 280));
